...
当前结论很明确：复现与诊断证据已经完整；R2-R15 当前作用域均已按门控闭环，尚无 `Primary-Candidate`，也未解锁 Round15 final freeze/test。Round12 发现 ensemble/oracle 互补空间但 learned lift 不稳；Round13 `ADWA-PANDA` 梯度可行但 D4 未打过 static aux 2.0 / generic DWA；Round14 因缺 split-safe OOF utility target 未启动 B/C。
...
- 后续 SOTA 与论文实验总方案：`docs/PANDA_SOTA冲刺与论文实验总方案.md`
- 文档索引：`docs/README.md`
...
6. Round11 UEA D4 消融显示 current utility-entropy per-sample aux allocation 未打过 static aux 2.0，且 reverse-utility control 与 best control 并列；当前融合方法只能作为诊断结果，不打开 D5。
7. Round12 ensemble audit 显示 PANDA+DWA equal-logit 只有 `+0.001633` Macro-F1，bootstrap 不支持稳健正收益；oracle any-correct 到 `0.983736/0.983740`，说明有互补但当前 learned/simple rules 不能稳定提取。
8. Round13 ADWA-PANDA D3.5 梯度可行，但 D4 best ADWA `0.933331/0.933333/0.980645` 低于 static aux 2.0、generic DWA 与 deterministic/same-budget/detached controls，不打开 D5。
9. Round14 OOF launch gate 因缺 split-safe OOF utility target 关闭 B/C；旧 train-only/val-diagnostic utility CSV 不能作为 OOF target。Round15 未启动，test 未打开。

后续不再直接沿 CUE/BUA/UEA/ADWA 当前实现续参。若继续追方法，必须先生成真正 K-fold OOF utility target 或设计本质不同的训练期机制；若继续追指标，优先补齐 DAMMFND/MMDFND 三 seed train/val 安全 logits 后重跑 Round12-style ensemble。
