...
当前结论很明确：复现与诊断证据已经完整；R2-R11 尚无 `Primary-Candidate`。下一阶段按 `docs/PANDA_SOTA冲刺与论文实验总方案.md` 推进 Round12-R15：先用 train/val-only ensemble / calibration 诊断 SOTA 冲榜空间，再以 `ADWA-PANDA / Anchored Dynamic Auxiliary-supervision PANDA` 追单模型方法，最后按门控决定是否重构 OOF utility calibration。
...
后续不再直接沿 CUE/BUA/UEA 续参，而是进入 Round12-R15：Round12 用多 seed / 多方法 logit ensemble 与 oracle disagreement audit 找可提升上界；Round13 设计 ADWA-PANDA，把 static aux 2.0 的稳定总预算和 DWA 的动态分支平衡结合起来；Round14 只在满足门控后做 split-safe OOF utility calibration；Round15 负责冻结配置和最终 test 协议。
