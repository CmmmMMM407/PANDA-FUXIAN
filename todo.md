...
总方案见 `docs/PANDA_SOTA冲刺与论文实验总方案.md`。下一阶段把 **SOTA 冲榜线** 和 **论文方法线** 分开推进：Round12 先做 train/val-only ensemble 上界诊断，Round13 优先做 `ADWA-PANDA / Anchored Dynamic Auxiliary-supervision PANDA` 单模型方法，Round14 只在满足门控后重构 OOF utility calibration，Round15 负责冻结配置和最终 test 协议。

所有任务继续默认 train/val-only；最终配置冻结前不导出、不打开、不分析 test。

- [ ] **Round12-A / 模型资产与 logits 盘点**
  在远端活代码与本地证据副本中盘点 PANDA、static aux 2.0、generic DWA、DAMMFND、MMDFND 的 seeds `42/2024/2026` train/val logits 或 probability artifact。输出 manifest，标记缺失项；只允许 train/val。

- [ ] **Round12-B / 缺失 train/val logits 安全导出**
  对缺失模型补导 train/val logits，禁止 test logits。优先补齐 `generic DWA` 三 seed、static aux 三 seed、PANDA reproduced 三 seed；DAMMFND/MMDFND 只在已有 reproduced checkpoint/logits 安全可用时加入。

- [ ] **Round12-C / Ensemble 上界与 diversity audit**
  跑 equal-weight logit average、validation-tuned convex weights、calibration-before-ensemble、oracle disagreement upper bound、pairwise error overlap / disagreement-correct audit。必须报告 single best、PANDA-only ensemble、PANDA+static、PANDA+DWA、PANDA+DAMMFND/MMDFND。

- [ ] **Round12-D / SOTA 冲榜线 Go/No-Go**
  `Go-to-Round15-final-freeze` 要求 val Macro-F1/Acc 超过 strongest single reproduced baseline，提升至少 `+0.002` Macro-F1 或有 paired bootstrap 支持，且提升来自至少两个 seed/方法族互补。否则 ensemble 只作为 diagnostic。

- [ ] **Round13-A / ADWA-PANDA 设计与 manifest**
  新建 `ADWA-PANDA` manifest。核心：static aux 2.0 作为总预算 anchor，DWA 只调 text/image/fusion auxiliary branches 相对权重，加入 clip 和 final-loss guard；不得使用 boundary hard gate。

- [ ] **Round13-B / ADWA D3.5 gradient sanity**
  验证 ADWA 梯度触达 `h_final`、`final_classifier`、branch features 和 aux heads；对照 static aux 2.0、generic DWA、GradNorm、PCGrad、CAGrad、detached aux、same-budget。

- [ ] **Round13-C / ADWA D4 seed42 smoke**
  Weibo-21 seed42 5 epoch train/val-only。候选至少包含 `adwa_clip_1p5_2p5`、`adwa_clip_1p0_2p5`、`adwa_final_guard`、`adwa_entropy_smoothed`；D4 必须同时打过 static aux 2.0 和 generic DWA 才能进入 D5。

- [ ] **Round13-D / ADWA D5 三 seed val 复核（门控）**
  只有 Round13-C 过线才启动。要求三 seed 每个 seed 不低于 deterministic，mean 优于 static aux 2.0 与 generic DWA，至少两个 seed 优于当 seed best strong control，flip audit W2C > C2W。

- [ ] **Round14-A / OOF Utility Calibration 启动判定**
  只有 Round12 显示明显 oracle/ensemble selection 空间，或 Round13 有正趋势但仍有 high-confidence wrong / branch disagreement 集中，才启动。禁止沿旧 CUE/BUA/UEA 直接续参。

- [ ] **Round14-B / OOF utility target 与 split-safe meta learner**
  在 train split 内部做 K-fold OOF utility target；meta feature 可用 logits、entropy、branch disagreement、calibrated confidence、PAD/source metadata、loss trajectory summary；禁止 test、val label 反向构造 target 或样本文本明文 shortcut。

- [ ] **Round14-C / OOF utility controls / moat**
  必须打过 Platt/temperature、final+aux stacking、confidence-only gate、shuffled utility、reverse utility、random utility、static aux 2.0、generic DWA。否则只写 diagnostic。

- [ ] **Round15 / 冻结配置与最终 test 协议（门控）**
  只有 Round12/13/14 至少一条线完成三 seed val 并写明最终配置才启动。冻结模型族、seed、ensemble 权重或单模型超参、calibration、early stopping、preprocessing、checkpoint 选择规则；最终 test 只允许冻结后打开一次。
